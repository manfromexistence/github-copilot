...
export function getCode(desiredLang: string): string | false {
  ...
  if ((langs as Record<string, string>)[desiredLang]) {
    return desiredLang;
  }
  const keys = Object.keys(langs).filter(key => {
    if (typeof (langs as Record<string, string>)[key] !== 'string') {
      return false;
    }
    const val = (langs as Record<string, string>)[key];
    return val && val.toLowerCase() === desiredLang;
  });
  return keys[0] || false;
}
